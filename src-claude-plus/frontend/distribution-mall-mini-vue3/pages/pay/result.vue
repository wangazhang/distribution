    ...
    <view class="pay-result-box ss-flex-col ss-row-center ss-col-center">
      ...
      <view class="btn-box ss-flex ss-row-center ss-m-t-50">
        <button
          ...
          @tap="sheep.$router.redirect('/pages/index/index')"
        >
          返回首页
        </button>
        <button
...
<script setup>
  ...
  function onOrder() {
    ...
    if (state.orderType === 'recharge') {
      sheep.$router.smartRedirect('/pages/pay/recharge-log');
    } else if (state.orderType === 'material_convert') {
      sheep.$router.smartRedirect('/yehu/pages/mine/mb-order/index');
    } else if (state.orderType === 'material_restock') {
      sheep.$router.smartRedirect('/yehu/pages/mine/mb-order/index');
    } else {
      sheep.$router.smartRedirect('/pages/order/list');
    }
  }
