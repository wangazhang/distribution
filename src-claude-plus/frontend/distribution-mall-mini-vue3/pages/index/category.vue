...
<script setup>
  ...
  async function getList() {
    ...
    const categoryTree = handleTree(data);
    state.categoryList = categoryTree;
    return categoryTree;
  }
  ...
  const refreshCategories = async (targetCategoryId) => {
    const currentCategoryId =
      targetCategoryId ?? state.categoryList?.[state.activeMenu]?.id ?? null;
    const categoryTree = await getList();
    ...
    const nextIndex =
      currentCategoryId !== null
        ? categoryTree.findIndex((category) => category.id === Number(currentCategoryId))
        : fallbackIndex;
    onMenu(nextIndex >= 0 ? nextIndex : fallbackIndex);
  };

  onLoad(async (params) => {
    await refreshCategories(params?.id);
  });
