...
<template>
  <s-layout class="wallet-wrap" title="佣金">
    ...
    <s-empty v-if="state.pagination.list.length === 0" icon="/static/data-empty.png" text="暂无数据"></s-empty>
...
<script setup>
  ...
  const WITHDRAW_BIZ_TYPES = [9999, 9998]; // 提现相关业务类型（申请、驳回）
  ...
  async function getLogList() {
    ...
    try {
      ...
      if (pageList.length > 0) {
        state.pagination.list = _.concat(state.pagination.list, pageList);
      }
      const total = data?.total ?? state.pagination.list.length;
      state.pagination.total = total;
      const pageSize = data?.pageSize || state.pagination.pageSize;
      const reachedEnd = pageList.length < pageSize || state.pagination.list.length >= total;
      state.loadStatus = reachedEnd ? 'noMore' : 'more';
      state.pagination.pageNo += 1;
    } catch (error) {
      console.error('getLogList error:', error);
      state.loadStatus = '';
    }
  }
  ...
  function isWithdrawRecord(bizType) {
    return WITHDRAW_BIZ_TYPES.includes(Number(bizType));
  }
  ...
  function openDetail(record) {
    ...
    if (isWithdrawRecord(record.bizType)) {
      if (!record.bizId) {
        sheep.$helper.toast('暂无法查看提现详情');
        return;
      }
      sheep.$router.go('/yehu/pages/mine/account/withdraw/detail/detail', { id: record.bizId });
      return;
    }
    if (!record.id) {
      return;
    }
    sheep.$router.go('/pages/commission/record-detail', { id: record.id });
  }
