    ...
    <view class="header-box ss-flex ss-row-center ss-col-center">
      <view class="card-box ui-BG-Main ui-Shadow-Main">
        ...
        <view class="ss-flex ss-row-between ss-col-center ss-m-t-64">
          ...
          <button class="ss-reset-button topup-btn" @tap="handleRecharge">充值</button>
        </view>
      </view>
    </view>
...
<style lang="scss" scoped>
  ...
  async function checkRechargeAvailable() {
    const { code, data } = await PayWalletApi.getWalletRechargePackageList();
    state.rechargeEnabled = code === 0 && Array.isArray(data) && data.length > 0;
  }

  function handleRecharge() {
    if (!state.rechargeEnabled) {
      uni.showToast({
        title: '功能已被管理员禁用',
        icon: 'none',
      });
