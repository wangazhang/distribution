    ...
    >
      ...
      <view class="order-list" v-for="item in state.orderInfo.items" :key="item.goods_id">
        <view class="order-card">
          <text style="color: #999999;padding-left: 20rpx;">订单项编号：{{ item.id }}</text>
          <s-goods-item
...
<script setup>
  ...
  const onCopyItemId = (itemId) => {
    if (!itemId) {
      return;
    }
    sheep.$helper.copyText(String(itemId));
  };
  ...
  async function onConfirm(orderId, ignore = false) {
    if (!ignore && canInvokeChannelConfirm()) {
      const extra = state.orderInfo.wechat_extra_data || {};
    ...
    uni.showModal({
      ...
      success: async function (res) {
        ...
        await submitConfirmReceive(orderId);
      },
    });
  }
  ...
  async function handleChannelConfirmResult(extraData) {
    ...
    if (status === 'success') {
      ...
      if (synced) {
        await getOrderDetail(pending.orderId);
        return;
      }
  ...
  async function syncChannelConfirmStatus(orderId) {
    try {
      const { code, data } = await OrderApi.checkChannelConfirm({ orderId });
