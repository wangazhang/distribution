...
/**
 * 便捷方法：保持与 Axios 类似的语法 request.get/post 等
 */
...
//
// // API级别的mock配置 - key为API路径，value为是否使用mock
// export const apiMockConfig = {};
//
// // 应用接口级别的mock控制 - 检查指定接口是否应该使用mock
// export const shouldUseMock = (url) => {
//   // 如果全局mock开关关闭，则不使用mock
//   console.log(`[API Mock配置] 当前全局mock状态为: ${useMock}`)
//   if (!useMock) return false;
//
//   // 如果API级别有明确的配置，则使用该配置
//   if (url in apiMockConfig) {
//     return apiMockConfig[url];
//   }
//
//   // 默认使用全局配置
//   return useMock;
...
//
// // 设置特定API的mock配置
// export const setApiMockConfig = (url, useMock) => {
//   apiMockConfig[url] = useMock;
//   console.log(`[API Mock配置] 已设置 ${url} 的mock状态为: ${useMock}`);
// }
//
// // 批量设置API的mock配置
// export const batchSetApiMockConfig = (config) => {
//   Object.entries(config).forEach(([url, value]) => {
//     apiMockConfig[url] = value;
//   });
//   console.log('[API Mock配置] 已批量更新接口mock配置');
// }
//
// // 获取当前的API mock配置
// export const getApiMockConfig = () => {
//   return { ...apiMockConfig };
// }
//
// // 重置所有API级别的mock配置
// export const resetApiMockConfig = () => {
//   Object.keys(apiMockConfig).forEach(key => {
//     delete apiMockConfig[key];
//   });
//   console.log('[API Mock配置] 已重置所有接口mock配置');
// }
