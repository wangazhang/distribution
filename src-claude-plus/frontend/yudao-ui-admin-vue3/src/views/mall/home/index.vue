<template>
  <doc-alert title="商城手册（功能开启）" url="https://doc.example.com/mall/build/" />
    ...
    <el-row :gutter="16" class="row">
      <el-col :md="6" :sm="12" :xs="24" :loading="loading">
        <ComparisonCard
          ...
          prefix="￥"
          :decimals="2"
          :value="toYuanNumber(orderComparison?.value?.orderPayPrice)"
          :reference="toYuanNumber(orderComparison?.reference?.orderPayPrice)"
        />
      </el-col>
      <el-col :md="6" :sm="12" :xs="24" :loading="loading">
        <ComparisonCard
...
/** 将分金额安全转换为数字类型的元金额，避免 CountTo 类型警告 */
const toYuanNumber = (price?: number | string | null) => {
  if (price === undefined || price === null) {
    return 0
  }
  const fen = typeof price === 'string' ? Number(price) : price
  if (Number.isNaN(fen)) {
    return 0
  }
  return fen / 100
