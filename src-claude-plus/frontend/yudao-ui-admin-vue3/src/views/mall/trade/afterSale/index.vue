<template>
  <doc-alert title="【交易】售后退款" url="https://doc.example.com/mall/trade-aftersale/" />
...
onMounted(async () => {
  if (route.query.orderNo) {
    queryParams.orderNo = route.query.orderNo as string
  }
  if (route.query.status) {
    queryParams.status = route.query.status as string
  }
  await getList()
  // 设置 statuses 过滤
...
watch(
  ...
  async (orderNo, prev) => {
    ...
    queryParams.orderNo = (orderNo as string) || null
    queryParams.pageNo = 1
    await getList()
  }
)
...
watch(
  ...
  async (status, prev) => {
    ...
    queryParams.status = (status as string) || '0'
    queryParams.pageNo = 1
    await getList()
  }
)
