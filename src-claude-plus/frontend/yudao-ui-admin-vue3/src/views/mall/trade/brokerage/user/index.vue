<template>
  ...
  <DistributionGraphDialog
    ref="distributionGraphDialogRef"
    @node-click="handleNodeClick"
    @open-brokerage-detail="handleOpenBrokerageDetail"
    @open-material-detail="handleOpenMaterialDetail"
  />
</template>
...
const openBrokerageDetailDialog = (row: BrokerageUserApi.BrokerageUserVO) => {
  console.log('[分销用户页] 列表操作触发 openBrokerageDetailDialog，row:', row)
...
/** 处理打开佣金详情事件 */
const handleOpenBrokerageDetail = (data: { userId: number; nickname: string }) => {
  console.log('[分销用户页] 父组件接收到佣金详情事件:', data)
  console.log('[分销用户页] brokerageDetailDialogRef 当前状态:', brokerageDetailDialogRef.value)

  try {
    if (brokerageDetailDialogRef.value) {
      console.log('[分销用户页] 准备调用佣金详情弹窗 open 方法')

      // 确保在下一个事件循环中执行，避免与父弹窗状态冲突
      nextTick(() => {
        console.log('[分销用户页] nextTick 中调用佣金详情弹窗 open')
        brokerageDetailDialogRef.value.open(data.userId, data.nickname)
        console.log('[分销用户页] 佣金详情弹窗 open 调用完成')
      })
    } else {
      console.error('[分销用户页] brokerageDetailDialogRef.value 为空，可能组件未正确挂载')
    }
  } catch (error) {
    console.error('[分销用户页] 打开佣金详情弹窗失败:', error)
  }
}

/** 处理打开物料详情事件 */
const handleOpenMaterialDetail = (data: { userId: number; nickname: string }) => {
  console.log('[分销用户页] 父组件接收到物料详情事件:', data)
  console.log('[分销用户页] materialDetailDialogRef 当前状态:', materialDetailDialogRef.value)

  try {
    if (materialDetailDialogRef.value) {
      console.log('[分销用户页] 准备调用物料详情弹窗 open 方法')

      // 确保在下一个事件循环中执行，避免与父弹窗状态冲突
      nextTick(() => {
        console.log('[分销用户页] nextTick 中调用物料详情弹窗 open')
        materialDetailDialogRef.value.open(data.userId, data.nickname)
        console.log('[分销用户页] 物料详情弹窗 open 调用完成')
      })
    } else {
      console.error('[分销用户页] materialDetailDialogRef.value 为空，可能组件未正确挂载')
    }
  } catch (error) {
    console.error('[分销用户页] 打开物料详情弹窗失败:', error)
  }
}
