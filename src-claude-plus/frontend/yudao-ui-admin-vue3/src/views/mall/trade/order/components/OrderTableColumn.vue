    ...
    <template #default="scope">
      ...
      >
        ...
        <el-table-column label="售后状态" prop="afterSaleStatus" width="120">
          <template #default="{ row }">
            <span
              v-if="isAfterSaleApplying(row.afterSaleStatus)"
              class="after-sale-link"
              @click="openAfterSaleRecord(scope.row)"
            >
              <dict-tag
...
const AFTER_SALE_APPLY_STATUS = 10
const { push } = useRouter()

const isAfterSaleApplying = (status?: number | null) =>
  status !== null && status !== undefined && status === AFTER_SALE_APPLY_STATUS

const openAfterSaleRecord = (order: OrderVO) => {
  if (!order?.no) {
    return
  }
  push({
