<template>
  <doc-alert title="【交易】交易订单" url="https://doc.example.com/mall/trade-order/" />
  <doc-alert title="【交易】购物车" url="https://doc.example.com/mall/trade-cart/" />
                ...
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item
                    v-if="
                      row.deliveryType === DeliveryTypeEnum.EXPRESS.type &&
                      row.payChannelCode &&
                      row.payChannelCode.toLowerCase().includes('wx') &&
...
const handleExport = async () => {
  try {
    await ElMessageBox.confirm('确认导出当前查询条件下的订单数据？', '提示', {
      type: 'warning'
    })
  } catch {
    return
  ...
  try {
    const data = await TradeOrderApi.exportOrder(unref(queryParams))
    download.excel(data, '交易订单.xls')
  } finally {
...
const handleChannelPush = async (row: TradeOrderApi.OrderVO) => {
  if (!row.id) {
    return
  }
  try {
    await ElMessageBox.confirm('确认推送渠道发货？', '提示', {
      type: 'warning'
    })
  } catch {
    return
  ...
  try {
    await TradeOrderApi.pushChannelShipping(row.id)
    ElMessage.success('渠道发货推送成功')
    await getList()
  } catch (error) {
    // 全局已处理错误提示
  }
...
const canRefund = (row: TradeOrderApi.OrderVO) =>
  Boolean(row.payStatus) && row.refundStatus !== TradeOrderRefundStatusEnum.ALL.status

const handleRefund = async (row: TradeOrderApi.OrderVO) => {
  if (!row?.id) {
    return
  }
  let refundReason = ''
  try {
    const { value } = await ElMessageBox.prompt('请输入退款原因（可留空）', '发起退款', {
  ...
  if (requireRefundPassword.value) {
    try {
      const { value } = await ElMessageBox.prompt('请输入退款密码', '身份校验', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputPlaceholder: '退款密码',
        inputType: 'password'
      })
      refundPassword = value?.trim() || ''
      if (!refundPassword) {
        ElMessage.warning('退款密码不能为空')
        return
      }
    } catch {
      return
