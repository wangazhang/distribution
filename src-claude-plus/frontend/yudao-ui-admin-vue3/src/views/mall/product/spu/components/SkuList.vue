<template>
  ...
  >
    ...
    <template v-for="level in props.memberLevelList" :key="`member-price-${level?.id}`">
      <el-table-column
        align="center"
        :label="`${level?.name || '会员'}价`"
        min-width="168"
      >
        <template #default="{ row }">
          <el-input-number
            :model-value="getMemberPrice(row, level?.id)"
            :min="0"
            :precision="2"
            :step="0.1"
            class="w-100%"
            controls-position="right"
            @update:model-value="(val) => setMemberPrice(row, level?.id, val)"
          />
        </template>
      </el-table-column>
    </template>
...
import type { LevelVO } from '@/api/member/level'
...
const props = defineProps({
  ...
  memberLevelList: {
    type: Array as PropType<LevelVO[]>,
    default: () => []
  },
...
/**
 * 深拷贝会员价配置，避免引用同一数组导致表单联动异常
 */
const cloneMemberPrices = (prices?: Sku['memberPrices']) => {
  return prices ? prices.map((item) => ({ ...item })) : []
}

/**
 * 根据会员等级编号获取当前行的会员价
 */
const getMemberPrice = (row: Sku, levelId?: number) => {
  if (!levelId || !row.memberPrices) {
    return 0
  }
  const target = row.memberPrices.find((item) => item.levelId === levelId)
  return target?.price ?? 0
}

/**
 * 更新指定等级的会员价，自动新增或删除配置
 */
const setMemberPrice = (row: Sku, levelId?: number, value?: number) => {
  if (!levelId) {
    return
  }
  row.memberPrices = row.memberPrices ? [...row.memberPrices] : []
  const targetIndex = row.memberPrices.findIndex((item) => item.levelId === levelId)
  if (value === undefined || value === null) {
    if (targetIndex !== -1) {
      row.memberPrices.splice(targetIndex, 1)
    }
    return
  }
  if (targetIndex !== -1) {
    row.memberPrices[targetIndex].price = value
  } else {
    row.memberPrices.push({ levelId, price: value })
  }
}
...
const batchAdd = () => {
  ...
  formData.value!.skus!.forEach((item) => {
    ...
    item.memberPrices = cloneMemberPrices(skuList.value[0].memberPrices)
  })
}
/** 校验商品属性属性值 */
