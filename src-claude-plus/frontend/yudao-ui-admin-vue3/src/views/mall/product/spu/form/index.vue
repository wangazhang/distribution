  ...
  if (id) {
    ...
    try {
      ...
      res.skus?.forEach((item) => {
        ...
        // 会员价同样需要完成分转元的回显，方便表单编辑
        item.memberPrices = item.memberPrices
          ? item.memberPrices.map((price) => ({
              ...price,
              price: isDetail.value ? floatToFixed2(price.price) : formatToFraction(price.price)
            }))
          : []
      })
      formData.value = res
...
const submitForm = async () => {
  ...
  try {
    ...
    if (!deepCopyFormData.type) {
      deepCopyFormData.type = 1
    }
    deepCopyFormData.skus!.forEach((item) => {
      ...
      // 会员价列表仅保留配置了等级的条目，同时将金额统一转为分
      item.memberPrices = item.memberPrices
        ?.filter((price) => price.levelId)
        ?.map((price) => ({
          levelId: price.levelId,
          price: convertToInteger(price.price)
        }))
    })
    // 处理轮播图列表
