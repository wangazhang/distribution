<!-- 商品中心 - 商品列表  -->
<template>
  <doc-alert title="【商品】商品 SPU 与 SKU" url="https://doc.example.com/mall/product-spu-sku/" />
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="商品类型" prop="type">
        <el-select v-model="queryParams.type" class="!w-240px" clearable placeholder="请选择商品类型">
          <el-option
  ...
  <ContentWrap>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column align="center" label="商品类型" min-width="120">
        <template #default="{ row }">
          {{ productTypeMap.get(row.type) ?? '-' }}
        </template>
      </el-table-column>
...
const productTypeMap = new Map(productTypeOptions.map((item) => [item.value, item.label]))
