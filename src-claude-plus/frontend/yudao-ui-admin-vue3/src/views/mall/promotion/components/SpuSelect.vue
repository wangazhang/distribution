...
const props = defineProps({
  ...
  selectableOnly: propTypes.bool.def(false) // 是否仅展示可选择的商品（排除售罄和回收站）
})
...
const queryParams = ref({
  pageNo: 1,
  pageSize: 10,
  tabType: props.selectableOnly ? undefined : 0,
  name: '',
  categoryId: null,
  createTime: [],
  selectableOnly: props.selectableOnly
}) // 查询参数
...
const ensureSpuRow = async (spuId: number) => {
  if (!spuId) {
    return undefined
  }
  ...
  let row = list.value.find((item) => item.id === spuId)
  if (row) {
    return row
  }
  try {
    const detail = (await ProductSpuApi.getSpu(spuId)) as ProductSpuApi.Spu
    if (detail) {
      row = detail
      list.value = [detail, ...list.value.filter((item) => item.id !== spuId)]
      return row
    }
  } catch (error) {
...
const presetSelection = async (spuId: number, skuIds?: number[]) => {
  const row = await ensureSpuRow(spuId)
  if (!row) {
    return
  }
  await nextTick()
  ...
  await nextTick()
  spuListRef.value?.toggleRowSelection(row, true)
  if (props.isSelectSku) {
    ...
    if (skuTable) {
      ...
      if (skuIds?.length) {
        const selectedSkus =
          spuData.value?.skus?.filter((sku) => sku.id && skuIds.includes(sku.id)) ?? []
        selectedSkus.forEach((sku) => {
          skuTable.toggleRowSelection(sku, true)
        })
      }
    }
  }
...
/** 打开弹窗 */
const open = async (options?: { spuId?: number; skuIds?: number[] }) => {
  dialogTitle.value = '商品选择'
  dialogVisible.value = true
  await nextTick()
  if (options?.spuId) {
    await presetSelection(options.spuId, options.skuIds)
  } else {
    resetSelection()
  }
}
...
watch(
  () => props.selectableOnly,
  (newVal) => {
    queryParams.value.tabType = newVal ? undefined : 0
    queryParams.value.selectableOnly = newVal
    resetSelection()
    getList()
  }
)
