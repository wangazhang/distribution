<template>
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-table-column prop="subMerchantId" label="子商户号" min-width="160" />
  ...
  <el-drawer v-model="detailVisible" title="资料详情" size="50%">
    <el-descriptions :column="2" border v-if="current">
      ...
      <el-descriptions-item label="驳回原因">
        <span>{{ current.rejectReason || '-' }}</span>
      </el-descriptions-item>
    </el-descriptions>
  </el-drawer>

  <el-dialog v-model="editVisible" title="修改资料" width="520px">
...
import { PayeaseSubMerchantApi, PayeaseSubMerchantRespVO } from '@/api/pay/payease/subMerchant'

const loading = ref(false)
const detailVisible = ref(false)
const editVisible = ref(false)
const list = ref<PayeaseSubMerchantRespVO[]>([])
const total = ref(0)
...
const getList = async () => {
  loading.value = true
  try {
    const data = await PayeaseSubMerchantApi.getPage(queryParams)
...
const openDetail = async (id: number) => {
  current.value = await PayeaseSubMerchantApi.get(id)
  detailVisible.value = true
}

const openEdit = async (id: number) => {
  const data = await PayeaseSubMerchantApi.get(id)
  current.value = data
...
const handleUpdate = async () => {
  await PayeaseSubMerchantApi.update(editForm)
...
const confirmSubmit = async (id: number) => {
  await ElMessageBox.confirm('确认提交至首信易进行审核吗？', '提示', {
    type: 'warning'
  })
  await PayeaseSubMerchantApi.submit(id)
