<template>
  <doc-alert title="支付功能开启" url="https://doc.example.com/pay/build/" />
  <!-- 搜索 -->
  <ContentWrap>
    <el-form
      ref="queryFormRef"
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column align="center" label="账户服务">
        <template #default="scope">
          <el-tag v-if="scope.row.accountChannelName" type="success" effect="plain">
            {{ scope.row.accountChannelName }}
          </el-tag>
      ...
      <el-table-column align="center" label="汇付天下配置">
        <el-table-column
          v-for="channel in huifuChannels"
          :key="channel.code"
          :label="channel.name.replace('汇付天下', '')"
          align="center"
        >
          <template #default="scope">
            <el-button
      ...
      <el-table-column align="center" label="首信易配置">
        <el-table-column
          v-for="channel in payeaseChannels"
          :key="channel.code"
          :label="channel.name.replace('首信易', '')"
          align="center"
        >
          <template #default="scope">
            <el-button
  ...
  <HuifuChannelForm ref="huifuFormRef" @success="getList" />
  <PayeaseChannelForm ref="payeaseFormRef" @success="getList" />
...
import HuifuChannelForm from './components/channel/HuifuChannelForm.vue'
import PayeaseChannelForm from './components/channel/PayeaseChannelForm.vue'
...
const huifuChannels = [
  PayChannelEnum.HUIFU_WX_LITE,
  PayChannelEnum.HUIFU_WX_NATIVE,
  PayChannelEnum.HUIFU_ALIPAY
]

const payeaseChannels = [
  PayChannelEnum.PAYEASE_WX_LITE,
  PayChannelEnum.PAYEASE_WX_NATIVE,
  PayChannelEnum.PAYEASE_ALIPAY,
  PayChannelEnum.PAYEASE_ACCOUNT
]
...
const huifuFormRef = ref()
const payeaseFormRef = ref()
...
const openChannelForm = async (row, payCode) => {
  ...
  if (payCode.indexOf('huifu_') === 0) {
    huifuFormRef.value.open(row.id, payCode)
    return
  }
  if (payCode.indexOf('payease_') === 0) {
    payeaseFormRef.value.open(row.id, payCode)
    return
  }
