...
export interface Sku {
  ...
  memberPrices?: MemberPrice[] // 会员价配置
}
...
// 获得商品 SPU 精简列表
export const getSpuSimpleList = async (name?: string) => {
  return request.get({ url: '/product/spu/list-all-simple', params: { name } })
}

// 获得商品 SPU 精简列表，基于商品名称模糊查询
export const getSpuSimpleListByName = async (name?: string) => {
  return request.get({ url: '/product/spu/list-simple-by-name', params: { name } })
}
