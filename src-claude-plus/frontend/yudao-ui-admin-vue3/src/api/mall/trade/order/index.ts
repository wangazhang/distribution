...
// 推送渠道发货
export const pushChannelShipping = async (id: number) => {
  return await request.put({
    url: `/trade/order/channel/push`,
    params: { id }
  })
}
...
// 后台发起订单退款
export const refundOrder = async (data: { id: number; reason?: string; password: string }) => {
  return await request.post({ url: `/trade/order/refund`, data })
}

export const getRefundConfig = async () => {
  return await request.get<boolean>({ url: '/trade/order/refund/config' })
}

// 导出交易订单
export const exportOrder = async (params: any) => {
  return await request.download({ url: `/trade/order/export-excel`, params })
}
