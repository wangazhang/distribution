package com.hissp.distribution.framework.apilog.core.filter;
...
import com.hissp.distribution.framework.web.core.filter.ApiRequestFilter;
...
import com.hissp.distribution.module.infra.api.logger.dto.ApiAccessLogCreateReqDTO;
...
import static com.hissp.distribution.framework.apilog.core.interceptor.ApiAccessLogInterceptor.ATTRIBUTE_HANDLER_METHOD;
...
public class ApiAccessLogFilter extends ApiRequestFilter {
    ...
    /**
     * 获取 HandlerMethod，直接通过 HandlerMapping 解析
     */
    private HandlerMethod getHandlerMethod(HttpServletRequest request) {
        try {
            // 先尝试从 request 属性中获取（如果 Interceptor 已经设置了）
            HandlerMethod handlerMethod = (HandlerMethod) request.getAttribute(ATTRIBUTE_HANDLER_METHOD);
            if (handlerMethod != null) {
                return handlerMethod;
            }

            // 如果 Interceptor 还没有执行，直接通过 HandlerMapping 获取
            ...
            for (HandlerMapping mapping : handlerMappings.values()) {
                try {
                    ...
                    // 忽略异常，继续尝试下一个 HandlerMapping
                }
            }
        } catch (Exception e) {
            log.warn("[getHandlerMethod][获取 HandlerMethod 失败]", e);
        }
        return null;
    }
