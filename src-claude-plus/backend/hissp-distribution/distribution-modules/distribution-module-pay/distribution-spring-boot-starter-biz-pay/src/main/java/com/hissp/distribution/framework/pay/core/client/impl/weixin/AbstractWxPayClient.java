package com.hissp.distribution.framework.pay.core.client.impl.weixin;
...
import com.hissp.distribution.framework.common.util.io.FileUtils;
import com.hissp.distribution.framework.common.util.object.ObjectUtils;
...
import com.hissp.distribution.framework.pay.core.client.dto.transfer.WxPayTransferPartnerNotifyV3Result;
...
import static com.hissp.distribution.framework.pay.core.client.impl.weixin.WxPayClientConfig.API_VERSION_V2;
import static com.hissp.distribution.framework.pay.core.client.impl.weixin.WxPayClientConfig.API_VERSION_V3;
...
public abstract class AbstractWxPayClient extends AbstractPayClient<WxPayClientConfig> {
    ...
    protected PayOrderInnerRespDTO doUnifiedOrder(PayOrderUnifiedInnerReqDTO reqDTO) throws Exception {
        try {
            ...
            return PayOrderInnerRespDTO.closedOf(errorCode, errorMessage,
     ...
     * @return 各支付渠道的返回结果
     */
    protected abstract PayOrderInnerRespDTO doUnifiedOrderV2(PayOrderUnifiedInnerReqDTO reqDTO)
            throws Exception;
     ...
     * @return 各支付渠道的返回结果
     */
    protected abstract PayOrderInnerRespDTO doUnifiedOrderV3(PayOrderUnifiedInnerReqDTO reqDTO)
     ...
     * @return 下单请求
     */
    protected WxPayUnifiedOrderRequest buildPayUnifiedOrderRequestV2(PayOrderUnifiedInnerReqDTO reqDTO) {
     ...
     * @return 下单请求
     */
    protected WxPayUnifiedOrderV3Request buildPayUnifiedOrderRequestV3(PayOrderUnifiedInnerReqDTO reqDTO) {
    ...
    @Override
    public PayOrderInnerRespDTO doParseOrderNotify(Map<String, String> headers, Map<String, String> params, String body) throws WxPayException {
        switch (config.getApiVersion()) {
            case API_VERSION_V2:
    ...
    private PayOrderInnerRespDTO doParseOrderNotifyV2(String body) throws WxPayException {
        // 1. 解析回调
        ...
        return PayOrderInnerRespDTO.of(status, response.getTransactionId(), response.getOpenid(), parseDateV2(response.getTimeEnd()),
                response.getOutTradeNo(), body);
    }

    private PayOrderInnerRespDTO doParseOrderNotifyV3(String body) throws WxPayException {
        // 1. 解析回调
        ...
        return PayOrderInnerRespDTO.of(status, result.getTransactionId(), openid, parseDateV3(result.getSuccessTime()),
    ...
    private PayOrderInnerRespDTO doGetOrderV2(String outTradeNo) throws WxPayException {
    ...
    private PayOrderInnerRespDTO doGetOrderV3(String outTradeNo) throws WxPayException {
        // 构建 WxPayUnifiedOrderRequest 对象
        ...
        return PayOrderInnerRespDTO.of(status, response.getTransactionId(), openid, parseDateV3(response.getSuccessTime()),
                outTradeNo, response);
    }
    ...
    protected PayRefundInnerRespDTO doUnifiedRefund(PayRefundUnifiedInnerReqDTO reqDTO) throws Throwable {
        try {
            ...
            String errorCode = getErrorCode(e);
            String errorMessage = getErrorMessage(e);
            return PayRefundInnerRespDTO.failureOf(errorCode, errorMessage,
                    reqDTO.getOutRefundNo(), e.getXmlString());
        }
    }

    private PayRefundInnerRespDTO doUnifiedRefundV2(PayRefundUnifiedInnerReqDTO reqDTO) throws Throwable {
        ...
        if (Objects.equals("SUCCESS", response.getResultCode())) { // V2 情况下，不直接返回退款成功，而是等待异步通知
            return PayRefundInnerRespDTO.waitingOf(response.getRefundId(),
                    reqDTO.getOutRefundNo(), response);
        }
        return PayRefundInnerRespDTO.failureOf(reqDTO.getOutRefundNo(), response);
    }

    private PayRefundInnerRespDTO doUnifiedRefundV3(PayRefundUnifiedInnerReqDTO reqDTO) throws Throwable {
        // 1. 构建 WxPayRefundRequest 请求
        ...
        if (Objects.equals("SUCCESS", response.getStatus())) {
            return PayRefundInnerRespDTO.successOf(response.getRefundId(), parseDateV3(response.getSuccessTime()),
                    reqDTO.getOutRefundNo(), response);
        }
    ...
    @Override
    public PayRefundInnerRespDTO doParseRefundNotify(Map<String, String> headers, Map<String, String> params, String body) throws WxPayException {
        switch (config.getApiVersion()) {
            case API_VERSION_V2:
    ...
    private PayRefundInnerRespDTO doParseRefundNotifyV2(String body) throws WxPayException {
        // 1. 解析回调
        ...
        if (Objects.equals("SUCCESS", result.getRefundStatus())) {
            return PayRefundInnerRespDTO.successOf(result.getRefundId(), parseDateV2B(result.getSuccessTime()),
                    result.getOutRefundNo(), response);
        }
        return PayRefundInnerRespDTO.failureOf(result.getOutRefundNo(), response);
    }

    private PayRefundInnerRespDTO parseRefundNotifyV3(String body) throws WxPayException {
        // 1. 解析回调
        ...
        // 2. 构建结果
        if (Objects.equals("SUCCESS", result.getRefundStatus())) {
            return PayRefundInnerRespDTO.successOf(result.getRefundId(), parseDateV3(result.getSuccessTime()),
                    result.getOutRefundNo(), response);
        }
        return PayRefundInnerRespDTO.failureOf(result.getOutRefundNo(), response);
    }
    ...
    @Override
    public PayTransferInnerRespDTO doParseTransferNotify(Map<String, String> headers,
                                                         Map<String, String> params,
                                                         String body) throws WxPayException {
        switch (config.getApiVersion()) {
            case API_VERSION_V3:
    ...
    private PayTransferInnerRespDTO parseTransferNotifyV3(String body) throws WxPayException {
        // 1. 解析回调
        ...
        if (Objects.equals("FINISHED", result.getBatchStatus())) {
            if (result.getFailNum() <= 0) {
                return PayTransferInnerRespDTO.successOf(result.getBatchId(), parseDateV3(result.getUpdateTime()),
                        result.getOutBatchNo(), response);
            }
        }
        return PayTransferInnerRespDTO.closedOf(result.getBatchStatus(), result.getCloseReason(), result.getOutBatchNo(), response);
    }

    @Override
    protected PayRefundInnerRespDTO doGetRefund(String outTradeNo, String outRefundNo) throws WxPayException {
        try {
            switch (config.getApiVersion()) {
                case API_VERSION_V2:
    ...
    private PayRefundInnerRespDTO doGetRefundV2(String outTradeNo, String outRefundNo) throws WxPayException {
        // 1. 构建 WxPayRefundRequest 请求
        ...
        if (!Objects.equals("SUCCESS", response.getResultCode())) {
            return PayRefundInnerRespDTO.waitingOf(null,
        ...
        if (refund == null) {
            return PayRefundInnerRespDTO.failureOf(outRefundNo, response);
        }
        switch (refund.getRefundStatus()) {
            case "SUCCESS":
                return PayRefundInnerRespDTO.successOf(refund.getRefundId(), parseDateV2B(refund.getRefundSuccessTime()),
                        outRefundNo, response);
            case "PROCESSING":
                return PayRefundInnerRespDTO.waitingOf(refund.getRefundId(),
                        outRefundNo, response);
    ...
    private PayRefundInnerRespDTO doGetRefundV3(String outTradeNo, String outRefundNo) throws WxPayException {
        // 1. 构建 WxPayRefundRequest 请求
    ...
    @Override
    protected PayTransferInnerRespDTO doUnifiedTransfer(PayTransferUnifiedInnerReqDTO reqDTO) throws WxPayException {
        ...
        // 2.2 创建返回结果
        return PayTransferInnerRespDTO.dealingOf(transferBatchesResult.getBatchId(), reqDTO.getOutTransferNo(), transferBatchesResult);
    }

    @Override
    protected PayTransferInnerRespDTO doGetTransfer(String outTradeNo, PayTransferTypeEnum type) throws WxPayException {
        ...
        if (Objects.equals("FINISHED", transferBatch.getBatchStatus())) {
            ...
            if (response.getTransferDetailList().stream().allMatch(detail -> Objects.equals("SUCCESS", detail.getDetailStatus()))) {
                return PayTransferInnerRespDTO.successOf(transferBatch.getBatchId(), parseDateV3(transferBatch.getUpdateTime()),
            ...
            if (response.getTransferDetailList().stream().anyMatch(detail -> Objects.equals("FAIL", detail.getDetailStatus()))) {
                return PayTransferInnerRespDTO.closedOf(transferBatch.getBatchStatus(), transferBatch.getCloseReason(),
                        transferBatch.getOutBatchNo(), response);
            }
        }
        ...
        return PayTransferInnerRespDTO.dealingOf(transferBatch.getBatchId(), transferBatch.getOutBatchNo(), response);
    }
