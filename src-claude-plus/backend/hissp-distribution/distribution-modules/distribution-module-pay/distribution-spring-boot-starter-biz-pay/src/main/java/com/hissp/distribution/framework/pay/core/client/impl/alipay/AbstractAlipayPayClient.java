package com.hissp.distribution.framework.pay.core.client.impl.alipay;
...
import static com.hissp.distribution.framework.common.exception.enums.GlobalErrorCodeConstants.*;
import static com.hissp.distribution.framework.common.exception.util.ServiceExceptionUtil.exception;
import static com.hissp.distribution.framework.common.exception.util.ServiceExceptionUtil.exception0;
import static com.hissp.distribution.framework.pay.core.client.impl.alipay.AlipayPayClientConfig.MODE_CERTIFICATE;
...
public abstract class AbstractAlipayPayClient extends AbstractPayClient<AlipayPayClientConfig> {
    ...
    /**
     * 构造支付关闭的 {@link PayOrderInnerRespDTO} 对象
     *
     * @return 支付关闭的 {@link PayOrderInnerRespDTO} 对象
     */
    protected PayOrderInnerRespDTO buildClosedPayOrderRespDTO(PayOrderUnifiedInnerReqDTO reqDTO, AlipayResponse response) {
        Assert.isFalse(response.isSuccess());
        return PayOrderInnerRespDTO.closedOf(response.getSubCode(), response.getSubMsg(),
                reqDTO.getOutTradeNo(), response);
    }

    @Override
    public PayOrderInnerRespDTO doParseOrderNotify(Map<String, String> headers, Map<String, String> params, String body) throws Throwable {
        // 1. 校验回调数据
        ...
        return PayOrderInnerRespDTO.of(status, bodyObj.get("trade_no"), bodyObj.get("seller_id"), parseTime(params.get("gmt_payment")),
    ...
    protected PayOrderInnerRespDTO doGetOrder(String outTradeNo) throws Throwable {
        ...
        return PayOrderInnerRespDTO.of(status, response.getTradeNo(), response.getBuyerUserId(), LocalDateTimeUtil.of(response.getSendPayDate()),
                outTradeNo, response);
    }
     ...
     */
    @Override
    protected PayRefundInnerRespDTO doUnifiedRefund(PayRefundUnifiedInnerReqDTO reqDTO) throws AlipayApiException {
        // 1.1 构建 AlipayTradeRefundModel 请求
        ...
        if (!response.isSuccess()) {
            ...
            if (ObjectUtils.equalsAny(response.getSubCode(), "ACQ.SYSTEM_ERROR", "SYSTEM_ERROR")) {
                return PayRefundInnerRespDTO.waitingOf(null, reqDTO.getOutRefundNo(), response);
            }
            return PayRefundInnerRespDTO.failureOf(response.getSubCode(), response.getSubMsg(), reqDTO.getOutRefundNo(), response);
        }
        // 2.2 创建返回结果
        // 支付宝只要退款调用返回 success，就认为退款成功，不需要回调。具体可见 parseNotify 方法的说明。
        // 另外，支付宝没有退款单号，所以不用设置
        return PayRefundInnerRespDTO.successOf(null, LocalDateTimeUtil.of(response.getGmtRefundPay()),
                reqDTO.getOutRefundNo(), response);
    }

    @Override
    public PayRefundInnerRespDTO doParseRefundNotify(Map<String, String> headers, Map<String, String> params, String body) {
    ...
    @Override
    protected PayRefundInnerRespDTO doGetRefund(String outTradeNo, String outRefundNo) throws AlipayApiException {
        ...
        if (!response.isSuccess()) {
            ...
            return PayRefundInnerRespDTO.waitingOf(null, outRefundNo, response);
        }
        // 2.2 创建返回结果
    ...
    @Override
    protected PayTransferInnerRespDTO doUnifiedTransfer(PayTransferUnifiedInnerReqDTO reqDTO) throws AlipayApiException {
        ...
        if (!response.isSuccess()) {
            ...
            if (ObjectUtils.equalsAny(response.getSubCode(),"PAYMENT_INFO_INCONSISTENCY", "SYSTEM_ERROR", "ACQ.SYSTEM_ERROR")) {
                return PayTransferInnerRespDTO.waitingOf(null, reqDTO.getOutTransferNo(), response);
            }
            return PayTransferInnerRespDTO.closedOf(response.getSubCode(), response.getSubMsg(),
            ...
            if (Objects.equals(response.getStatus(), "DEALING")) { // 转账到银行卡会出现 "DEALING"  处理中
                return PayTransferInnerRespDTO.dealingOf(response.getOrderId(), reqDTO.getOutTransferNo(), response);
            }
            return PayTransferInnerRespDTO.successOf(response.getOrderId(), parseTime(response.getTransDate()),
    ...
    protected PayTransferInnerRespDTO doGetTransfer(String outTradeNo, PayTransferTypeEnum type) throws Throwable {
        ...
        if (response.isSuccess()) {
            ...
            if (Objects.equals(response.getStatus(), "DEALING")) { // 转账到银行卡会出现 "DEALING" 处理中
                return PayTransferInnerRespDTO.dealingOf(response.getOrderId(), outTradeNo, response);
            }
            return PayTransferInnerRespDTO.successOf(response.getOrderId(), parseTime(response.getPayDate()),
                    response.getOutBizNo(), response);
        } else {
            // 当出现 SYSTEM_ERROR, 转账可能成功也可能失败。 返回 WAIT 状态. 后续 job 会轮询, 或相同 outBizNo 重新发起转账
            ...
            if (ObjectUtils.equalsAny(response.getSubCode(), "ORDER_NOT_EXIST", "SYSTEM_ERROR", "ACQ.SYSTEM_ERROR")) {
                return PayTransferInnerRespDTO.waitingOf(null, outTradeNo, response);
            }
            return PayTransferInnerRespDTO.closedOf(response.getSubCode(), response.getSubMsg(),
                    outTradeNo, response);
        }
