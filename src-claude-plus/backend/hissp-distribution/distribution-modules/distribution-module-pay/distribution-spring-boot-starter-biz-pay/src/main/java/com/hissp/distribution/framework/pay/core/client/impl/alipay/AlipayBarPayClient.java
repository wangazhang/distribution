...
public class AlipayBarPayClient extends AbstractAlipayPayClient {
    ...
    @Override
    public PayOrderInnerRespDTO doUnifiedOrder(PayOrderUnifiedInnerReqDTO reqDTO) throws AlipayApiException {
        ...
        if ("10000".equals(response.getCode())) { // 免密支付
            ...
            return PayOrderInnerRespDTO.successOf(response.getTradeNo(), response.getBuyerUserId(), successTime,
                            response.getOutTradeNo(), response)
                    .setDisplayMode(displayMode).setDisplayContent("");
        }
        // 大额支付，需要用户输入密码，所以返回 waiting。此时，前端一般会进行轮询
        return PayOrderInnerRespDTO.waitingOf(displayMode, "",
