...
import static com.hissp.distribution.framework.pay.core.enums.order.PayOrderStatusRespEnum.CLOSED;
import static com.hissp.distribution.framework.pay.core.enums.order.PayOrderStatusRespEnum.WAITING;
...
public class AlipayBarPayClientTest extends AbstractAlipayClientTest {
    ...
    public void testUnifiedOrder_success() throws AlipayApiException {
        ...
        // 准备请求参数
        PayOrderUnifiedInnerReqDTO reqDTO = buildOrderUnifiedReqDTO(notifyUrl, outTradeNo, price);
        Map<String, String> extraParam = new HashMap<>();
        extraParam.put("auth_code", authCode);
        reqDTO.setChannelExtras(extraParam);

        // 调用方法
        PayOrderInnerRespDTO resp = client.unifiedOrder(reqDTO);
        // 断言
    ...
    public void testUnifiedOrder_code10000Success() throws AlipayApiException {
        ...
        // 准备请求参数
        String authCode = randomString();
        PayOrderUnifiedInnerReqDTO reqDTO = buildOrderUnifiedReqDTO(randomURL(), outTradeNo, randomInteger());
        Map<String, String> extraParam = new HashMap<>();
        extraParam.put("auth_code", authCode);
        reqDTO.setChannelExtras(extraParam);
    ...
    public void testUnifiedOrder_emptyAuthCode() {
        // 准备参数
        PayOrderUnifiedInnerReqDTO reqDTO = buildOrderUnifiedReqDTO(randomURL(), randomString(), randomInteger());
