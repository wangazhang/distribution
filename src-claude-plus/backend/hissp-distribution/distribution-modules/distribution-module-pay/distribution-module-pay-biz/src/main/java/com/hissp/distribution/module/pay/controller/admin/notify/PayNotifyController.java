package com.hissp.distribution.module.pay.controller.admin.notify;
...
import com.hissp.distribution.module.pay.controller.admin.notify.vo.PayNotifyTaskDetailRespVO;
import com.hissp.distribution.module.pay.controller.admin.notify.vo.PayNotifyTaskPageReqVO;
import com.hissp.distribution.module.pay.controller.admin.notify.vo.PayNotifyTaskRespVO;
import com.hissp.distribution.module.pay.convert.notify.PayNotifyTaskConvert;
...
import static com.hissp.distribution.module.pay.enums.ErrorCodeConstants.CHANNEL_NOT_FOUND;
...
public class PayNotifyController {
    ...
    public String notifyOrder(@PathVariable("channelId") Long channelId,
        ...
        // 2. 解析通知数据
        PayOrderInnerRespDTO notify = payClient.parseOrderNotify(headers,params, body);
    ...
    public String notifyRefund(@PathVariable("channelId") Long channelId,
        ...
        // 2. 解析通知数据
        PayRefundInnerRespDTO notify = payClient.parseRefundNotify(headers, params, body);
    ...
    public String notifyTransfer(@PathVariable("channelId") Long channelId,
        ...
        // 2. 解析通知数据
        PayTransferInnerRespDTO notify = payClient.parseTransferNotify(headers, params, body);
        payTransferService.notifyTransfer(channelId, notify);
        return "SUCCESS";
    }

    @PostMapping(value = "/account-declaration/{channelId}")
    @Operation(summary = "首信易账户服务【入网】回调")
    @PermitAll
    public String notifyAccountDeclaration(@PathVariable("channelId") Long channelId,
                                           @RequestHeader(required = false) Map<String, String> headers,
                                           @RequestParam(required = false) Map<String, String> params,
                                           @RequestBody(required = false) String body) {
        try {
            paySettleAccountService.handleDeclarationNotify(channelId, headers, params, body);
            return "SUCCESS";
        } catch (Exception ex) {
            log.error("[notifyAccountDeclaration][处理失败 channelId={}]", channelId, ex);
            return "FAILED";
        }
    }
