package com.hissp.distribution.module.pay.framework.pay.config;
...
@ConfigurationProperties(prefix = "distribution.pay")
@Validated
@Data
public class PayProperties {
     ...
     *
     * 回调顺序：支付渠道（支付宝支付、微信支付） => distribution-module-pay 的 orderNotifyUrl 地址 => 业务的 PayAppDO.orderNotifyUrl 地址
     */
     ...
     *
     * 回调顺序：支付渠道（支付宝支付、微信支付） => distribution-module-pay 的 refundNotifyUrl 地址 => 业务的 PayAppDO.notifyRefundUrl 地址
     */
    @NotEmpty(message = "支付回调地址不能为空")
    @URL(message = "支付回调地址的格式必须是 URL")
     ...
     *
     * 回调顺序：支付渠道（支付宝支付、微信支付） => distribution-module-pay 的 transferNotifyUrl 地址 => 业务的 PayAppDO.transferNotifyUrl 地址
     */
     ...
     * 首信易账户渠道编号
     */
    private Long payeaseAccountChannelId;

    /**
     * 默认的账户服务应用标识。当未显式传入应用时，会优先使用该应用绑定的账户服务渠道
     */
    private String defaultAccountAppKey;

    /**
     * 账户服务入网/申报回调地址（可选）。为空时默认沿用 orderNotifyUrl
     */
    private String accountDeclarationNotifyUrl;
