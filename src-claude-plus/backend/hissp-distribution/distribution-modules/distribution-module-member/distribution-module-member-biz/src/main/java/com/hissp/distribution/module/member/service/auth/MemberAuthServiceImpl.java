package com.hissp.distribution.module.member.service.auth;

import cn.hutool.core.lang.Assert;
import cn.hutool.core.util.ObjectUtil;
import com.hissp.distribution.framework.common.enums.CommonStatusEnum;
import com.hissp.distribution.framework.common.enums.TerminalEnum;
...
import com.hissp.distribution.module.member.convert.auth.AuthConvert;
...
import com.hissp.distribution.module.system.api.logger.LoginLogApi;
import com.hissp.distribution.module.system.api.logger.dto.LoginLogCreateReqDTO;
import com.hissp.distribution.module.system.api.oauth2.OAuth2TokenApi;
import com.hissp.distribution.module.system.api.oauth2.dto.OAuth2AccessTokenCreateReqDTO;
import com.hissp.distribution.module.system.api.oauth2.dto.OAuth2AccessTokenRespDTO;
import com.hissp.distribution.module.system.api.sms.SmsCodeApi;
...
import com.hissp.distribution.module.system.api.social.dto.SocialWxPhoneNumberInfoRespDTO;
import com.hissp.distribution.module.system.enums.logger.LoginLogTypeEnum;
import com.hissp.distribution.module.system.enums.logger.LoginResultEnum;
import com.hissp.distribution.module.system.enums.oauth2.OAuth2ClientConstants;
...
public class MemberAuthServiceImpl implements MemberAuthService {
    ...
    public AppAuthLoginRespVO weixinMiniAppLogin(AppAuthWeixinMiniAppLoginReqVO reqVO) {
        ...
        log.info("[weixinMiniAppLogin][微信小程序登录，获取到手机号: {}]", mobile);
        ...
        if (socialUser != null && socialUser.getUserId() != null) {
            // 情况1：已存在社交用户绑定，获取对应的用户
            user = userService.getUser(socialUser.getUserId());
            Assert.notNull(user, "社交用户绑定的会员用户不存在");
            openid = socialUser.getOpenid();

            log.info("[weixinMiniAppLogin][找到已绑定的社交用户: userId={}, openid={}, currentMobile={}, newMobile={}]",
                user.getId(), openid, user.getMobile(), mobile);

            // 如果用户没有手机号，则填充手机号
            if (ObjectUtil.isEmpty(user.getMobile()) && ObjectUtil.isNotEmpty(mobile)) {
                log.info("[weixinMiniAppLogin][用户无手机号，填充手机号: userId={}, mobile={}]", user.getId(), mobile);
                userService.updateUserMobile(user.getId(), mobile);
                user.setMobile(mobile); // 更新内存中的对象
            }

        } else {
            // 情况2：未找到社交用户绑定，需要创建或查找用户并绑定
            log.info("[weixinMiniAppLogin][未找到社交用户绑定，处理用户创建或绑定]");

            // 2.1 根据手机号查找是否已有用户
            user = userService.getUserByMobile(mobile);
            if (user == null) {
                // 2.2 创建新用户
                log.info("[weixinMiniAppLogin][创建新用户: mobile={}]", mobile);
                user = userService.createUserIfAbsent(mobile, getClientIP(), TerminalEnum.WECHAT_MINI_PROGRAM.getTerminal());
            } else {
                log.info("[weixinMiniAppLogin][找到已存在用户: userId={}, mobile={}]", user.getId(), mobile);
            }
            ...
            log.info("[weixinMiniAppLogin][完成社交用户绑定: userId={}, openid={}]", user.getId(), openid);
        }
