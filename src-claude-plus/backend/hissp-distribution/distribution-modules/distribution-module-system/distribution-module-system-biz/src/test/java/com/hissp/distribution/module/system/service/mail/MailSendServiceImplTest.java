package com.hissp.distribution.module.system.service.mail;
...
import com.hissp.distribution.framework.test.core.util.RandomUtils;
import com.hissp.distribution.module.system.dal.dataobject.mail.MailAccountDO;
import com.hissp.distribution.module.system.dal.dataobject.mail.MailTemplateDO;
import com.hissp.distribution.module.system.dal.dataobject.user.AdminUserDO;
import com.hissp.distribution.module.system.mq.message.mail.MailSendMessage;
import com.hissp.distribution.module.system.mq.producer.mail.MailProducer;
...
//                .setFrom("奥特曼 <ydym_test@example.com>")
                .setFrom("ydym_test@example.com") // 邮箱地址
                .setHost("smtp.163.com").setPort(465).setSslEnable(true) // SMTP 服务器
                .setAuth(true).setUser("ydym_test@example.com").setPass("WBZTEINMIFVRYSOE".toCharArray()); // 登录账号密码
        String messageId = MailUtil.send(mailAccount, "7685413@example.com", "主题", "内容", false);
    ...
    public void testDoSendMail_success() {
        try (final MockedStatic<MailUtil> mailUtilMock = mockStatic(MailUtil.class)) {
            ...
            // mock 方法（获得邮箱账号）
            MailAccountDO account = randomPojo(MailAccountDO.class, o -> o.setMail("7685@example.com"));
            ...
            mailUtilMock.when(() -> MailUtil.send(
                    argThat(mailAccount -> {
                        assertEquals("芋艿 <7685@example.com>", mailAccount.getFrom());
