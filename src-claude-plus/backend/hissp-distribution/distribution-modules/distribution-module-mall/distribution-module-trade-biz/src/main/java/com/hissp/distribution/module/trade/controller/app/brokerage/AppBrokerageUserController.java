...
public class AppBrokerageUserController {
    ...
    public CommonResult<AppBrokerageUserRespVO> getBrokerageUser() {
        Optional<BrokerageUserDO> user =
            Optional.ofNullable(brokerageUserService.getOrCreateBrokerageUser(getLoginUserId()));
        // 返回数据
        AppBrokerageUserRespVO respVO = new AppBrokerageUserRespVO()
            ...
            .setTotalBrokeragePrice(user.map(BrokerageUserDO::getTotalBrokeragePrice).orElse(0));
    ...
    public CommonResult<AppBrokerageUserMySummaryRespVO> getBrokerageUserSummary() {
        ...
        Integer yesterdayPrice = brokerageRecordService.getSummaryAmountByUserIdAndBizTypesBetween(userId,
            Arrays.asList(BrokerageRecordBizTypeEnum.ORDER_ADD

                    ),
            BrokerageRecordStatusEnum.SETTLEMENT, beginTime, endTime);
        // 统计用户提现的佣金
        Integer withdrawPrice = brokerageWithdrawService
            .getWithdrawSummaryListByUserId(Collections.singleton(userId),
                asList(BrokerageWithdrawStatusEnum.AUDIT_SUCCESS, BrokerageWithdrawStatusEnum.WITHDRAW_SUCCESS))
    ...
    public CommonResult<PageResult<AppBrokerageUserRankByUserCountRespVO>>
        getBrokerageUserRankPageByUserCount(AppBrokerageUserRankPageReqVO pageReqVO) {
        // 分页查询
        PageResult<AppBrokerageUserRankByUserCountRespVO> pageResult =
            brokerageUserService.getBrokerageUserRankPageByUserCount(pageReqVO);
        // 拼接数据
        Map<Long, MemberUserRespDTO> userMap =
            memberUserApi.getUserMap(convertSet(pageResult.getList(), AppBrokerageUserRankByUserCountRespVO::getId));
    ...
    public CommonResult<PageResult<AppBrokerageUserRankByPriceRespVO>>
        getBrokerageUserChildSummaryPageByPrice(AppBrokerageUserRankPageReqVO pageReqVO) {
        // 分页查询
        PageResult<AppBrokerageUserRankByPriceRespVO> pageResult =
            brokerageRecordService.getBrokerageUserChildSummaryPageByPrice(pageReqVO);
        // 拼接数据
        Map<Long, MemberUserRespDTO> userMap =
            memberUserApi.getUserMap(convertSet(pageResult.getList(), AppBrokerageUserRankByPriceRespVO::getId));
    ...
    public CommonResult<PageResult<AppBrokerageUserChildSummaryRespVO>>
        getBrokerageUserChildSummaryPage(AppBrokerageUserChildSummaryPageReqVO pageReqVO) {
        PageResult<AppBrokerageUserChildSummaryRespVO> pageResult =
            brokerageUserService.getBrokerageUserChildSummaryPage(pageReqVO, getLoginUserId());
    ...
    public CommonResult<Integer> getRankByPrice(@RequestParam("times") @DateTimeFormat(
        pattern = FORMAT_YEAR_MONTH_DAY_HOUR_MINUTE_SECOND) LocalDateTime[] times) {
    ...
    public CommonResult<AppBrokerageTeamMemberPageRespVO> getTeamMemberList(@Valid AppBrokerageTeamMemberPageReqVO pageReqVO) {
        return success(brokerageUserService.getTeamMemberList(pageReqVO, getLoginUserId()));
    }

    @GetMapping("/team-overview")
    @Operation(summary = "获取团队总览（统计及等级筛选项）")
    public CommonResult<AppBrokerageTeamOverviewRespVO> getTeamOverview() {
        return success(brokerageUserService.getTeamOverview(getLoginUserId()));
    }
