package com.hissp.distribution.module.trade.service.price.calculator;
...
import com.hissp.distribution.module.promotion.api.discount.DiscountActivityApi;
import com.hissp.distribution.module.promotion.api.discount.dto.DiscountProductRespDTO;
import com.hissp.distribution.module.promotion.enums.common.PromotionDiscountTypeEnum;
import com.hissp.distribution.module.promotion.enums.common.PromotionTypeEnum;
...
import static com.hissp.distribution.framework.common.util.number.MoneyUtils.calculateRatePrice;
import static com.hissp.distribution.module.trade.service.price.calculator.TradePriceCalculatorHelper.formatPrice;
...
public class TradeDiscountActivityPriceCalculator implements TradePriceCalculator {
    ...
    public Integer calculateVipPrice(MemberLevelRespDTO level,
                                      TradePriceCalculateRespBO.OrderItem orderItem) {
        if (level == null || CommonStatusEnum.isDisable(level.getStatus())) {
            return 0;
        }
        // 先尝试使用 SKU 自定义的会员价，如果没有才回退到折扣系数
        Integer customVipPrice = calculateMemberLevelVipPrice(level, orderItem);
        if (customVipPrice != null) {
            return customVipPrice;
        }
        if (level.getDiscountPercent() == null) {
            return 0;
        }
     ...
     * @param level 会员等级
     * @param orderItem 交易项
     * @return 自定义会员价优惠，null 表示无需走自定义计算
     */
    private Integer calculateMemberLevelVipPrice(MemberLevelRespDTO level,
                                                 TradePriceCalculateRespBO.OrderItem orderItem) {
        if (CollUtil.isEmpty(orderItem.getMemberPrices())) {
            return null;
        }
        ProductSkuRespDTO.MemberPrice memberPrice = CollUtil.findOne(orderItem.getMemberPrices(),
                price -> ObjectUtil.equal(price.getLevelId(), level.getId()));
        if (memberPrice == null || memberPrice.getPrice() == null) {
            return null;
        }
        Integer baseTotalPrice = ObjectUtil.defaultIfNull(orderItem.getPayPrice(),
                orderItem.getPrice() == null || orderItem.getCount() == null
                        ? null
                        : orderItem.getPrice() * orderItem.getCount());
        if (baseTotalPrice == null || baseTotalPrice <= 0) {
            return 0;
        }
        int count = ObjectUtil.defaultIfNull(orderItem.getCount(), 1);
        int memberTotalPrice = memberPrice.getPrice() * count;
        if (memberTotalPrice >= baseTotalPrice) {
            return 0;
        }
        return baseTotalPrice - memberTotalPrice;
    }
