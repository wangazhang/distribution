package com.hissp.distribution.module.trade.service.price;
...
import com.hissp.distribution.module.product.api.category.ProductCategoryApi;
...
import com.hissp.distribution.module.promotion.api.reward.RewardActivityApi;
import com.hissp.distribution.module.promotion.api.reward.dto.RewardActivityMatchRespDTO;
import com.hissp.distribution.module.promotion.enums.common.PromotionTypeEnum;
import com.hissp.distribution.module.trade.controller.app.order.vo.AppTradeProductSettlementRespVO;
...
import com.hissp.distribution.module.trade.service.price.calculator.TradeDiscountActivityPriceCalculator;
import com.hissp.distribution.module.trade.service.price.calculator.TradePriceCalculator;
...
import static com.hissp.distribution.module.product.enums.ErrorCodeConstants.SKU_STOCK_NOT_ENOUGH;
import static com.hissp.distribution.module.trade.enums.ErrorCodeConstants.PRICE_CALCULATE_PAY_PRICE_ILLEGAL;
...
public class TradePriceServiceImpl implements TradePriceService {
    ...
    public List<AppTradeProductSettlementRespVO> calculateProductPrice(Long userId, List<Long> spuIds) {
        ...
        // 1.1.1 是否无营销品
        if (productSpuApi.isNoneMarketingProduct(spuIds)) {
            //返回一个对应类型的 结果
            return convertList(spuIds, spuId -> new AppTradeProductSettlementRespVO().setSpuId(spuId));
        }
        // 1.2 获得会员等级
        ...
        return convertList(spuIds, spuId -> {
            ...
            List<AppTradeProductSettlementRespVO.Sku> skuVOList = convertList(skuList, sku -> {
                AppTradeProductSettlementRespVO.Sku skuVO = new AppTradeProductSettlementRespVO.Sku()
                        .setId(sku.getId());
                // 组合计算对象：count 仅用于比较 SKU 层级优惠（限时折扣 / 会员价）
                TradePriceCalculateRespBO.OrderItem orderItem = new TradePriceCalculateRespBO.OrderItem()
                        .setPrice(sku.getPrice()).setPayPrice(sku.getPrice()).setCount(1)
                        .setMemberPrices(sku.getMemberPrices());
                // 计算限时折扣的优惠价格
