...
public interface BrokerageWithdrawMapper extends BaseMapperX<BrokerageWithdrawDO> {
    ...
    default PageResult<BrokerageWithdrawDO> selectPage(BrokerageWithdrawPageReqVO reqVO) {
        return selectPage(reqVO, new LambdaQueryWrapperX<BrokerageWithdrawDO>()
                ...
                .orderByDesc(BrokerageWithdrawDO::getCreateTime)
                .orderByDesc(BrokerageWithdrawDO::getId));
    }

    default List<BrokerageWithdrawDO> selectList(BrokerageWithdrawExportReqVO reqVO) {
        return selectList(new LambdaQueryWrapperX<BrokerageWithdrawDO>()
                .eqIfPresent(BrokerageWithdrawDO::getUserId, reqVO.getUserId())
                .eqIfPresent(BrokerageWithdrawDO::getType, reqVO.getType())
                .likeIfPresent(BrokerageWithdrawDO::getName, reqVO.getName())
                .eqIfPresent(BrokerageWithdrawDO::getAccountNo, reqVO.getAccountNo())
...
//                .groupBy(BrokerageWithdrawDO::getUserId));
    ...
    default List<BrokerageWithdrawDO> selectListByStatus(Integer status) {
        return selectList(BrokerageWithdrawDO::getStatus, status);
    }

    default List<BrokerageWithdrawDO> selectLatestListByUserId(Long userId, Collection<Integer> types) {
        return selectList(new LambdaQueryWrapperX<BrokerageWithdrawDO>()
                .eq(BrokerageWithdrawDO::getUserId, userId)
                .inIfPresent(BrokerageWithdrawDO::getType, types)
                .orderByDesc(BrokerageWithdrawDO::getCreateTime)
                .orderByDesc(BrokerageWithdrawDO::getId));
    }
