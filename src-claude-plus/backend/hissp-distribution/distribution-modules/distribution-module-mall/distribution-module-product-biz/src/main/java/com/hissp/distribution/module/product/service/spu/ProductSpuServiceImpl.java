package com.hissp.distribution.module.product.service.spu;
...
import com.hissp.distribution.module.product.controller.admin.spu.vo.ProductSkuSaveReqVO;
import com.hissp.distribution.module.product.controller.admin.spu.vo.ProductSpuPageReqVO;
import com.hissp.distribution.module.product.controller.admin.spu.vo.ProductSpuSaveReqVO;
import com.hissp.distribution.module.product.controller.admin.spu.vo.ProductSpuUpdateStatusReqVO;
...
import com.hissp.distribution.module.product.dal.mysql.spu.ProductSpuMapper;
...
import com.hissp.distribution.module.product.service.brand.ProductBrandService;
import com.hissp.distribution.module.product.service.category.ProductCategoryService;
import com.hissp.distribution.module.product.service.sku.ProductSkuService;
...
import static com.hissp.distribution.module.product.dal.dataobject.category.ProductCategoryDO.CATEGORY_LEVEL;
import static com.hissp.distribution.module.product.enums.ErrorCodeConstants.*;
...
public class ProductSpuServiceImpl implements ProductSpuService {
    ...
    public Long createSpu(ProductSpuSaveReqVO createReqVO) {
        ...
        if (spu.getType() == null) {
            spu.setType(ProductSpuTypeEnum.getDefault());
        }
        // 初始化 SPU 中 SKU 相关属性
    ...
    public Boolean isNoneMarketingProduct(List<Long> spuIds) {
        ...
        //新建一个 Long 的 分类 list 初始化放一个 88L 78L 用List的初始化方式，后续可以扩展
        List<Long> careerCategoryIds = new ArrayList<Long>(List.of(88L));
        // 获得产品spu 获得产品分类路径 根据路径中是否存在88分类，判断是否属于 无营销品
        List<ProductSpuDO> spuList = getSpuList(spuIds);
        //获取 spuList中 每个元素的 商品分类，组成新的Long list数组 categoryIds
        List<Long> categoryIds = spuList.stream().map(ProductSpuDO::getCategoryId).toList();
        List<Long> productCategoryParentPathList = categoryService.getProductCategoryParentPathById(categoryIds);
        //productCategoryParentPathList 是否包含 careerCategoryIds中的元素，只要一个存在就是true
        return productCategoryParentPathList.stream().anyMatch(careerCategoryIds::contains);
    }
    @Override
    public Boolean isCareerProduct(List<Long> spuIds) {
        //新建一个 Long 的 分类 list 初始化放一个 88L 78L 用List的初始化方式，后续可以扩展
        List<Long> careerCategoryIds = new ArrayList<Long>(List.of(MbConstants.MbProductInfo.MB_AGENT_OBTAIN_GOODS_CATEGORY_ROOT_ID));
        // 获得产品spu 获得产品分类路径 根据路径中是否存在88分类，判断是否属于 无营销品
        List<ProductSpuDO> spuList = getSpuList(spuIds);
        //获取 spuList中 每个元素的 商品分类，组成新的Long list数组 categoryIds
        List<Long> categoryIds = spuList.stream().map(ProductSpuDO::getCategoryId).toList();
    ...
    public Boolean isMbProduct(List<Long> spuIds) {
        //新建一个 Long 的 分类 list 初始化放一个 88L 78L 用List的初始化方式，后续可以扩展
        List<Long> careerCategoryIds = new ArrayList<Long>(List.of(MbConstants.MbProductInfo.MB_NORMAL_GOODS_CATEGORY_ROOT_ID));
        // 获得产品spu 获得产品分类路径 根据路径中是否存在88分类，判断是否属于 无营销品
        List<ProductSpuDO> spuList = getSpuList(spuIds);
        //获取 spuList中 每个元素的 商品分类，组成新的Long list数组 categoryIds
        List<Long> categoryIds = spuList.stream().map(ProductSpuDO::getCategoryId).toList();
    ...
    @Override
    public List<ProductSpuDO> getSpuSimpleList(String name) {
        return productSpuMapper.selectListByName(name);
    }
