package com.hissp.distribution.module.product.service.sku;
...
import com.hissp.distribution.module.product.convert.sku.ProductSkuConvert;
...
import com.hissp.distribution.module.product.dal.dataobject.sku.ProductSkuDO;
import com.hissp.distribution.module.product.dal.mysql.sku.ProductSkuMapper;
import com.hissp.distribution.module.product.service.property.ProductPropertyService;
import com.hissp.distribution.module.product.service.property.ProductPropertyValueService;
...
public class ProductSkuServiceImpl implements ProductSkuService {
     ...
     * @param sku sku 对象
     */
    private void validateMemberPrices(ProductSkuSaveReqVO sku) {
        if (CollUtil.isEmpty(sku.getMemberPrices())) {
            return;
        }
        Set<Long> levelIds = new HashSet<>();
        sku.getMemberPrices().forEach(memberPrice -> {
            // 逐条校验：必须携带会员等级，且同一个等级不可重复配置
            if (memberPrice.getLevelId() == null || !levelIds.add(memberPrice.getLevelId())) {
                throw exception(SKU_MEMBER_PRICE_LEVEL_DUPLICATE);
            }
            // 金额需大于等于 0，前端输入为元，这里校验的是分
            if (memberPrice.getPrice() == null || memberPrice.getPrice() < 0) {
                throw exception(SKU_MEMBER_PRICE_INVALID);
            }
        });
    }
