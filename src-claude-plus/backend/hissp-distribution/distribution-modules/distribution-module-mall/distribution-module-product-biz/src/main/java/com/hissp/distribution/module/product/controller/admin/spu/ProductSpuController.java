package com.hissp.distribution.module.product.controller.admin.spu;
...
import com.hissp.distribution.module.product.controller.admin.spu.vo.*;
...
public class ProductSpuController {
    ...
    public CommonResult<ProductSpuRespVO> getSpuDetail(@RequestParam("id") Long id) {
        ...
        return success(BeanUtils.toBean(spu, ProductSpuRespVO.class).setSkus(BeanUtils.toBean(skus, ProductSkuRespVO.class)));
    }
    ...
    public CommonResult<List<ProductSpuSimpleRespVO>> getSpuSimpleList(
        ...
        List<ProductSpuDO> list = productSpuService.getSpuSimpleList(name);
        // 降序排序后，返回给前端
    ...
    public CommonResult<List<ProductSpuRespVO>> getSpuList(@RequestParam("spuIds") Collection<Long> spuIds) {
        ...
        List<ProductSkuDO> skuList = productSkuService.getSkuListBySpuId(spuIds);
        Map<Long, List<ProductSkuDO>> skuMap = skuList.stream().collect(Collectors.groupingBy(ProductSkuDO::getSpuId));
        return success(spuList.stream().map(spu -> {
